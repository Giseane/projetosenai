...
public class BancoController implements ContaCorrente {
	...
	/*
	 * public void aplicarJurosPoupanca() { List<ContaCorrentePF> contas =
	 * (List<ContaCorrentePF>) bancoRepository.findAll();
	 * 
	 * for (ContaCorrentePF cp : contas) { if (cp.getAccountType() ==
	 * AccountType.CONTA_POUPANCA) { Double saldo = cp.getSaldo(); Double juro =
	 * saldo * 0.001; // 0.1% de juros Double novoSaldo = saldo + juro;
	 * cp.setSaldo(novoSaldo); bancoRepository.save(cp); } } }
	 */
}